...
function cachedFetch(key, url) {
  ...
  const promise = (async () => {
    ...
    } catch { /* private mode, etc. — fall through to network */ }
...
const loadBook = (code, bookNum) =>
  cachedFetch(`bible/${code}/${bookNum}@${DATA_VERSION}`, `/data/bibles/${code}/${bookNum}.json`);
...
function NotesPane({ lang, refPos, highlight, scroller }) {
  ...
  useEffect(() => {
    ...
    loadNotes(lang, bookNum, refPos.chapter)
      ...
      .catch(e => { if (alive) { setNotes({}); setError(e.message); } });
    return () => { alive = false; };
  ...
  useEffect(() => {
    ...
    if (key) scroller.current?.querySelector(`[data-v="${keyStart(key)}"]`)
      ?.scrollIntoView({ block: 'center', behavior: 'smooth' });
  ...
  if (!keys.length) {
    return (
      <p className="dim">
        No {lang === 'es' ? 'Spanish' : 'English'} notes yet for {byId[refPos.book].en} {refPos.chapter}.
      </p>
    );
  }
  ...
  return keys.map(key => {
    ...
    return (
      <div key={key} data-v={keyStart(key)} className={isHit ? 'note hit' : 'note'}>
        <span className="note-ref">{refPos.chapter}:{key}</span>
        {notes[key].map((para, i) =>
          para.t === 'hl'
            ? <blockquote key={i} className="note-hl">{para.x}</blockquote>
            : <p key={i} className="note-p">{para.x}</p>
        )}
      </div>
    );
  });
  ...
  return (
    <div className="app">
      <form className="jump" onSubmit={submit} autoComplete="off">
        <button type="button" onClick={() => step(-1)} aria-label="Previous chapter">‹</button>
        <div className="field">
          <input
            value={query}
            ...
            <ul className="menu">
              {options.map(o => (
                <li key={o.id}>
                  <button
                    type="button"
                    onMouseDown={() => go({ book: o.id, chapter: Math.min(o.chapter, o.chapters), verse: o.verse })}
                  >
        ...
        <button type="button" onClick={() => step(1)} aria-label="Next chapter">›</button>
      </form>
